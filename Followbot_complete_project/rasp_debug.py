...
# 近距离锁头参数
STOP_TURN_HEIGHT = 0.55   

# 距离保持参数
TARGET_HEIGHT_MIN = 0.40  
TARGET_HEIGHT_MAX = 0.60  
...
def main():
    ...
    print(f"=== System Ready. Smart Return Logic Enabled. ===")
        ...
        while system_state["running"]:
            ...
            if current_mode == "STANDBY":
                ...
                if elapsed > 3.0:
                    ...
                    # ==================================================
                    # [优化逻辑] 决定下一步：是倒车(Return)还是直接切换
                    # ==================================================
                    
                    # 场景 A: 只有一个候选人 -> 直接去决断，不用倒车
                    if len(candidates_queue) == 1:
                        print(">> Only 1 candidate. Skipping return.")
                        system_state["mode"] = "DECIDING"
                    
                    else:
                        # 场景 B: 有多个候选人 -> 检查视野
                        # 我们需要在这里运行一次 YOLO 看看是否能看到所有人
                        check_results = model(frame, classes=[0], verbose=False)
                        box_count = len(check_results[0].boxes) if check_results[0].boxes else 0
                        print(f">> Visibility Check: See {box_count} / Need {len(candidates_queue)}")

                        # 如果当前能看到的人数 >= 候选人总数，说明视野足够，不需要倒车
                        if box_count >= len(candidates_queue):
                            print(">> All targets visible. Skipping return.")
                            curr_cand_idx += 1
                        ...
                        # 否则，因为被当前人挡住了，需要倒车拉开视野
                        else:
                            print(">> View blocked. Backing up...")
                            system_state["mode"] = "RETURNING"
                            action_timer = time.time()

            elif current_mode == "RETURNING":
                # 只进行直线后退，绝对不原地转圈
                # 因为已经确认是“视野被挡住”才进来的，所以必须退
                robot_move(BACKUP_SPEED, BACKUP_SPEED)
                elapsed = time.time() - action_timer
                cv2.putText(frame, "Widening View...", (10, 60), 0, 0.8, (0,0,255), 2)
